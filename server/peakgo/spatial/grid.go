// Package spatial provides the high-performance spatial grid engine used for
// proximity queries and AOI neighborhood tracking.
//
// # Peak Go contract
//
// All grid queries use pooled slices to avoid heap allocations on hot paths.
// The SpatialGrid type is designed for per-MapWorker isolation — each map
// owns its own grid instance via MapWorker.SpatialGrid.
package spatial
...
// GlobalGrid is the singleton spatial registry used by all systems.
// Initialized via NewGlobalGrid() at server startup.
// NOTE: In the per-MapWorker architecture, each map should use its own
// SpatialGrid instance. GlobalGrid is maintained for backward compatibility.
var GlobalGrid = NewGrid()

// NewGrid creates a fresh SpatialGrid with empty chunk/entity maps.
func NewGrid() *SpatialGrid {
	return &SpatialGrid{
...
// worldToChunk is an internal variant used by SpatialGrid methods directly,
// avoiding any potential confusion with the exported WorldToChunk.
func worldToChunk(pos ecs.PositionComponent) ChunkKey {
	return ChunkKey{
		MapID: pos.MapID,
		X:     int(math.Floor(float64(pos.X) / chunkSize)),
