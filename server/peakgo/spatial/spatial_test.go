...
func setupEntity(t testing.TB, name string, typ spatial.EntityType, mapID, x, z int) ecs.Entity {
	id := ecs.GlobalRegistry.NewEntity()

	// Lưu ý: Trường Metadata nội bộ của ECS vẫn dùng chuỗi (string),
	// ta gọi phương thức .String() của Enum để tương thích mượt mà.
	ecs.GlobalRegistry.SetMetadata(id, ecs.MetadataComponent{Name: name, Type: typ.String()})
...
func TestGetNearestPlayerFindsClosest(t *testing.T) {
	...
	monster := setupEntity(t, "Orc", spatial.EntityMonster, 1, 50, 50)
	near := setupEntity(t, "NearPlayer", spatial.EntityPlayer, 1, 52, 50) // khoảng cách = 2
	far := setupEntity(t, "FarPlayer", spatial.EntityPlayer, 1, 60, 50)   // khoảng cách = 10
...
func TestGetNearestMonster(t *testing.T) {
	...
	player := setupEntity(t, "Hero", spatial.EntityPlayer, 1, 10, 10)
	nearMob := setupEntity(t, "Slime", spatial.EntityMonster, 1, 11, 10) // khoảng cách = 1
	farMob := setupEntity(t, "Dragon", spatial.EntityMonster, 1, 25, 10) // khoảng cách = 15
...
func TestGetNearestReturnsNothingOutOfRange(t *testing.T) {
	monster := setupEntity(t, "Orc2", spatial.EntityMonster, 1, 10, 10)
	player := setupEntity(t, "Player2", spatial.EntityPlayer, 1, 90, 90) // Quá xa
...
func TestCountInRadiusAndSemanticWrappers(t *testing.T) {
	origin := setupEntity(t, "Origin", spatial.EntityMonster, 1, 30, 30)
	p1 := setupEntity(t, "P1", spatial.EntityPlayer, 1, 31, 30)  // khoảng cách = 1
	p2 := setupEntity(t, "P2", spatial.EntityPlayer, 1, 32, 30)  // khoảng cách = 2
	m1 := setupEntity(t, "M1", spatial.EntityMonster, 1, 33, 30) // khoảng cách = 3
	defer cleanupEntities(origin, p1, p2, m1)

	// Đã sửa: Kiểm tra song song cả hàm Generic và hàm ngữ nghĩa (Semantic API) mới
	playerCount := spatial.CountInRadius(origin, 5.0, spatial.EntityPlayer)
	if playerCount != 2 {
	...
	monsterCount := spatial.CountInRadius(origin, 5.0, spatial.EntityMonster)
	...
	allCount := spatial.CountInRadius(origin, 5.0, spatial.EntityAny)
	if allCount != 3 {
...
func TestIsAnyInRadiusAndSemanticWrappers(t *testing.T) {
	origin := setupEntity(t, "Check", spatial.EntityMonster, 1, 20, 20)
	player := setupEntity(t, "Nearby", spatial.EntityPlayer, 1, 21, 20)
	defer cleanupEntities(origin, player)

	if !spatial.IsAnyInRadius(origin, 3.0, spatial.EntityPlayer) {
	...
	if spatial.IsAnyInRadius(origin, 0.5, spatial.EntityPlayer) {
...
func TestFilterInRadius(t *testing.T) {
	origin := setupEntity(t, "Center", spatial.EntityPlayer, 1, 0, 0)
	m1 := setupEntity(t, "Mob1", spatial.EntityMonster, 1, 2, 2)
	m2 := setupEntity(t, "Mob2", spatial.EntityMonster, 1, 3, 3)
	p1 := setupEntity(t, "Friend", spatial.EntityPlayer, 1, 1, 1)
	defer cleanupEntities(origin, m1, m2, p1)

	// Lọc danh sách Monster
	var targets []ecs.Entity
	targets = spatial.FilterInRadius(origin, 5.0, spatial.EntityMonster, targets)
	if len(targets) != 2 {
...
func TestIsInRadius(t *testing.T) {
	a := setupEntity(t, "A", spatial.EntityPlayer, 1, 10, 10)
	b := setupEntity(t, "B", spatial.EntityMonster, 1, 13, 14) // khoảng cách hình học = 5
...
func TestDistanceBetween(t *testing.T) {
	a := setupEntity(t, "A", spatial.EntityPlayer, 1, 0, 0)
	b := setupEntity(t, "B", spatial.EntityPlayer, 1, 3, 4) // tam giác Ai Cập 3-4-5 -> bình phương = 25
	defer cleanupEntities(a, b)
...
func TestSameMap(t *testing.T) {
	a := setupEntity(t, "MapA1", spatial.EntityPlayer, 1, 5, 5)
	b := setupEntity(t, "MapA2", spatial.EntityPlayer, 1, 6, 6)
	c := setupEntity(t, "MapB1", spatial.EntityPlayer, 2, 5, 5)
...
func TestEdgeCasesNonExistentEntities(t *testing.T) {
	...
	if count := spatial.CountInRadius(invalidID, 10.0, spatial.EntityAny); count != 0 {
		t.Errorf("CountInRadius should return 0, got %d", count)
	}

	if any := spatial.IsAnyInRadius(invalidID, 10.0, spatial.EntityAny); any {
...
func TestSpatialZeroAllocations(t *testing.T) {
	origin := setupEntity(t, "Center", spatial.EntityPlayer, 1, 10, 10)
	target := setupEntity(t, "Target", spatial.EntityMonster, 1, 11, 11)
	defer cleanupEntities(origin, target)

	// Đảm bảo SpatialGrid nội bộ đã được khởi động và phân bố vùng đệm
	_ = spatial.CountInRadius(origin, 10.0, spatial.EntityMonster)
...
func runSpatialBenchmark(b *testing.B, entityCount int) {
	...
	monster := setupEntity(b, "BenchOrc", spatial.EntityMonster, 1, 50, 50)
	players := make([]ecs.Entity, entityCount)
	for i := range players {
		// Phân bố người chơi rải rác xung quanh vị trí quái vật
		players[i] = setupEntity(b, "P", spatial.EntityPlayer, 1, 50+(i%20), 50+(i/20))
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = spatial.CountInRadius(monster, 20.0, spatial.EntityPlayer)
	}
}
...
func BenchmarkGetNearestMonster_Scale100(b *testing.B) {
	world.InitializeCollisionMaps()
	player := setupEntity(b, "Hero", spatial.EntityPlayer, 1, 50, 50)
	monsters := make([]ecs.Entity, 100)
	for i := range monsters {
		monsters[i] = setupEntity(b, "M", spatial.EntityMonster, 1, 50+(i%10), 50+(i/10))
	}
	defer func() {
